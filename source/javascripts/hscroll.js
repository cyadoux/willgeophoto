...
var HtinyScrolling = {
		...
		scroll: function(targ) {
			...
			if (HtinyScrolling.requestedX > left) {
				...
				var offset = Math.max(15, Math.min(endDistance, HtinyScrolling.maxStep));
			} else {
...
var scrollTips = {
	...
	init: function() {
		...
		addEvent(left, 'mouseover', function() {
			this.dx = setInterval('scrollTips.arrowScroll(0)', 100);
			return false;
		});
		addEvent(left, 'mouseout', function() {
			clearInterval(this.dx);
			return false;
		});
		
		var right = document.getElementById('right');
		addEvent(right, 'mouseover', function() {
			this.dx = setInterval('scrollTips.arrowScroll(1)', 100);
			return false;
		});
		addEvent(right, 'mouseout', function() {
			clearInterval(this.dx);
			return false;
		});
	},
